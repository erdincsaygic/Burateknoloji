...
namespace StilPay.UI.Admin.Infrastructures
{
    public static class ExtensionMethods
    {
        ...
        public static void SaveSms(this ISession session, string key, string operationType, int confirmCode, byte counter = 0)
        {
            session.Remove(operationType);

            var json = JsonConvert.SerializeObject(new Sms { Key = key, OperationType = operationType, SmsDate = DateTime.Now, Counter = counter, ConfirmCode = confirmCode });
