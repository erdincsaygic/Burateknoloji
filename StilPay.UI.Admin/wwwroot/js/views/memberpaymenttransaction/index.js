$(document).ready(function () {
    ...
    $("#btnExcel").click(function () {
        downloadExcel("Bireysel-Üye-Ödeme-Hareketleri", mainData);
    });
...
function getData() {
    $('#Table').DataTable({
        ...
        "ajax": {
            "url": "/MemberPaymentTransaction/GetData",
            "type": "POST",
            "data": function (d) {
        ...
        "columnDefs": [
            ...
            },
            {
                ...
                "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    $(nTd).attr('class', 'text-center');
                    $(nTd).html(sData = '<a href="/MemberPaymentTransaction/Edit/' + sData + '" target="_blank" >Detay</a>');
                }
            }
        ],
...
var Table = function () {
    ...
    var initData = function () {
        $.ajax({
            type: "POST",
            url: 'MemberPaymentTransaction/Gets/',
            ...
            success: function (list) {
                ...
                if (list)
                    $.each(list, function (i, item) {
                        grid.push([item.cDate, item.transactionNr, item.member, item.phone, item.bank, formatDate(item.actionDate) + ' ' + item.actionTime, item.senderName, item.amount.toFixed(2), item.status, item.modifier, item.id]);
                        mainData.push({
