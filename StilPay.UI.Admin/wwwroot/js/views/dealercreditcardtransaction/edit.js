var urlLIST = '/dealercreditcardtransaction/index';
...
$(document).ready(function () {
    ...
    $("#sendAgainCallbackBtn").click(function () {
        $('#mdlCallbackConfirm').modal('show');
    });

    $("#akOdeTransactionQueryBtn").click(function () {
        $('#mdlAKODETransactionQuery').modal('show');
    });

    $('#mdlAKODETransactionQuery').on('hidden.bs.modal', function () {
        $("#transactionQueryResponseStatus").val('');
        $("#transactionQueryResponseMessage").val('');
    });
...
function onSuccessSendCallbackAgain(response) {
    if (response.status === "ERROR")
        alertify.notify(response.message, 'error', 5, function () { }).dismissOthers();
    else {
        var otbl = $('#Table').DataTable();
        var newData = [
            formatDateTime(response.data.cDate),
            response.data.company,
            response.data.serviceType,
            response.data.transactionNr,
            response.data.transactionType,
            response.data.responseStatus,
            response.data.callback
        ];
        otbl.row.add(newData).draw();

        // mainData'ya da ekle
        mainData.push(response.data);
        $("#mdlCallbackConfirm").modal("hide");
        alertify.notify(response.message, 'success', 5, function () { }).dismissOthers();
    }
}


function onSuccessAKODETransactionQuery(response) {
    if (response.status === "ERROR")
        alertify.notify(response.message, 'error', 5, function () { }).dismissOthers();
    else {
        if (response.data && response.data != null && response.data != undefined && response.data.length > 0) {
            $("#transactionQueryResponseStatus").val(response.data[0].status);
            $("#transactionQueryResponseMessage").val(response.data[0].message);
        }
    }
}
