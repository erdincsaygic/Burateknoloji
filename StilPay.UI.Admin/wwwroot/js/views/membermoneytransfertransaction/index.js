$(document).ready(function () {
    ...
    $("#btnExcel").click(function () {
        downloadExcel("Bireysel-Üye-Para-Transfer-Hareketleri", mainData);
    });
...
function getData() {
    $('#Table').DataTable({
        ...
        "ajax": {
            "url": "/MemberMoneyTransferTransaction/GetData",
            "type": "POST",
            "data": function (d) {
...
var Table = function () {
    ...
    var initData = function () {
        $.ajax({
            type: "POST",
            url: 'MemberMoneyTransferTransaction/Gets/',
            ...
            success: function (list) {
                ...
                if (list)
                    $.each(list, function (i, item) {
                        grid.push([item.cDate, item.transactionNr, item.member, item.phone, item.amount.toFixed(2), item.costTotal.toFixed(2), item.receiverPhone, item.receiverName ?? '>>Alıcı Üye Sistemde Kayıtlı Değil<<', item.status, item.modifier, item.id]);
                        mainData.push({
