    ...
    $('#Table').DataTable({
        ...
        "initComplete": function (settings, json) {
            if (json.data != null && json.data.length > 0) {
                $("#divNet").text(json.data[0].balance.toString().replace(".", ",") + ' TL');
                $("#divDate").text($("#dtStartDate").val() + ' - ' + $("#dtEndDate").val());
                $("#divTotal").text(json.data[0].balanceTotal.toString().replace(".", ",") + ' / ' + json.recordsFiltered + ' Ad');
                $("#divCommission").text(json.data[0].balanceCommission.toString().replace(".", ",") + ' / ' + json.data[0].commissionCount + ' Ad');
                $("#divRebate").text(json.data[0].rebateTotal.toString().replace(".", ",") + ' / ' + json.data[0].rebateTotalCount + ' Ad');
            } else {
                $("#divNet").text(0, 00 + ' TL');
                $("#divDate").text($("#dtStartDate").val() + ' - ' + $("#dtEndDate").val());
        ...
        "columnDefs": [
            ...
            },
            {
                "aTargets": [2],
                "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    if (parseInt(oData.idActionType) === 10 || parseInt(oData.idActionType) === 20 || parseInt(oData.idActionType) === 70 || parseInt(oData.idActionType) === 100 || parseInt(oData.idActionType) === 140 || parseInt(oData.idActionType) === 150)
                ...
                if (list)
                    $.each(list, function (i, item) {
                        ...
                        mainData.push({
                            ...
                            İşlemÜcreti: item.transactionTotal,
...
//    var num = number.toString().split(".").pop();
//    if (num.length == 4)
//        var newNumber = num.substring(num.length - 2, 0);
//    if (num.length == 3)
//        var newNumber = num.substring(num.length - 1, 0);

//    return parseInt(number, 10) + "." + newNumber;
//}