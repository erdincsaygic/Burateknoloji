...
function getData() {
    $('#TableManuel').DataTable({
        ...
        "ajax": {
            "url": "/CreditCardPaymentNotification/GetDataManuel",
            "type": "POST",
        ...
        "rowCallback": function (row, data) {
            $(row).on('click', function () {
                ...
                if (detailsRow.length === 0) {
                    ...
                    loadAllDetails(data.encryptedCardNumber, detailsRow);
                } else {
                    detailsRow.toggle();
                }
            });
    ...
    $('#TableAuto').DataTable({
        ...
        "ajax": {
            "url": "/CreditCardPaymentNotification/GetDataAuto",
            "type": "POST",
            //"dataSrc": function (data) {
            //    var datas = [];
            //    for (const [key, value] of Object.entries(data.data)) {
...
function loadAllDetails(cardNumber, detailsRow) {
    showLoading();
    $.ajax({
        url: '/DealerTransactionQuery/CreditCardDetailedSearchList',
        type: 'POST',
        data: { CardNumber: cardNumber, length: 100, start: 0, search: null, status: 2 }, 
        success: function (response) {
            var detailContainer = detailsRow.find('.detail-container'); 
            var detailHtml = '';

            if (response.data && response.data.length > 0) {
                detailHtml += '<table class="table table-bordered detail-table"><thead><tr><th style="font-weight: bold;">Tarih</th><th style="font-weight: bold;">Üye İşyeri</th><th style="font-weight: bold;">İşlem No</th><th style="font-weight: bold;">Üye İşyeri İşlem No</th><th style="font-weight: bold;">Gönderici Adı</th><th style="font-weight: bold;">Gönderici Telefon</th><th style="font-weight: bold;">Kart Numarası</th><th style="font-weight: bold;">Tutar</th></tr></thead><tbody>';

                response.data.forEach(function (detail) {
                    detailHtml += '<tr><td>' + formatDateTime(detail.cDate) + '</td><td>' + detail.company + '</td><td>' + detail.transactionNr + '</td><td>' + detail.transactionID + '</td><td>' + detail.senderName + '</td><td>' + detail.senderPhone + '</td><td>' + detail.cardNumber + '</td><td>' + detail.amount.toFixed(2) + '</td></tr>';
                });
