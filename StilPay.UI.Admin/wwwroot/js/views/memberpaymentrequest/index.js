...
$(document).ready(function () {
    ...
    $("#btnExcel").click(function () {
        downloadExcel("Bireysel-Üye-Ödeme-Bildirimleri", mainData);
    });
...
function getData() {
    $('#Table').DataTable({
        ...
        "ajax": {
            "url": "/MemberPaymentRequest/GetData",
            "type": "POST",
        },
        "columns": [
        ...
        "columnDefs": [
            ...
            },
            {
                ...
                "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                    $(nTd).attr('class', 'text-center');
                    $(nTd).html(sData = '<a href="/MemberPaymentRequest/Edit/' + sData + '" target="_blank" >Detay</a>');
                }
            }
        ],
...
var Table = function () {
        ...
        var otable = table.dataTable({
            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                        $(nTd).attr('class', 'text-center');
                        $(nTd).html(sData = '<a href="/MemberPaymentRequest/Edit/' + sData + '" >Detay</a>');
                    }
                }
            ],
    ...
    var initData = function () {
        ...
        $.get('/memberpaymentrequest/gets', function (list) {
            var otbl = $('#Table').DataTable();
            if (list)
                $.each(list, function (i, item) {
                    otbl.rows.add([[item.cDate, item.transactionNr, item.member, item.phone, item.bank, formatDate(item.actionDate) + ' ' + item.actionTime, item.senderName, item.amount.toFixed(2), item.status, item.id]]);
                    mainData.push({
