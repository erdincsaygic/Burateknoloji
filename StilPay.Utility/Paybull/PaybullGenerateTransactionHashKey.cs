...
{
    public class PaybullGenerateTransactionHashKey
    {
        public static string GenerateTransactionHashKey(string merchant_key, string date, string invoice_id, string currency_id, string paymentmethodid, string minamount, string maxamount, string transactionState)
        {
            string data = date + '|' + invoice_id + '|' + currency_id + '|' + paymentmethodid + '|' + minamount + '|' + maxamount + '|' + transactionState;
            Random mt_rand = new Random();
            string iv = Sha1Hash(mt_rand.Next().ToString()).Substring(0, 16);
            string password = Sha1Hash(merchant_key);
