...
{
    public class LidioPosCancelRequest
    {
        public static GenericResponseDataModel<LidioPosCancelRequestResponseModel> CancelRequest(LidioPosCancelRequestModel lidioPosCancelRequestModel, bool IsForeignCard = false)
        {
            try
            {
                ...
                var body = JsonConvert.SerializeObject(lidioPosCancelRequestModel);
                request.AddStringBody(body, DataFormat.Json);

                var response = client.Execute(request);
                var deserialize = JsonConvert.DeserializeObject<LidioPosCancelRequestResponseModel>(response.Content);

                if (response.IsSuccessStatusCode)
                {
                    return new GenericResponseDataModel<LidioPosCancelRequestResponseModel>
                    {
