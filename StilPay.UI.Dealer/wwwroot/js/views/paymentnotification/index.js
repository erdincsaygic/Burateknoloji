            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
                        ...
                        $(nTd).attr('class', 'text-center');
                        $(nTd).html(sData = '<a href="/Process/Detail/' + oData.isAutoNotification ? 10 : 20 + '/' + sData + '"  target="_blank">Detay</a>');
                    }
                }
            ],
            ...
            success: function (list) {
                ...
                if (list)
                    $.each(list, function (i, item) {
                        grid.push([item.cDate, item.transactionNr, item.member, item.phone, item.senderName, item.senderIdentityNr, item.amount.toFixed(2), item.status, item.mDate, item.description, item.id, item.isAutoNotification]);
                        _list = list;
                        mainData.push({
