var urlEDIT = '/BankAccount/Edit/__id__';
var urlDELETE = '/BankAccount/Drop';
...
var Table = function () {
    ...
    var initData = function () {
        ...
        $.get('/bankaccount/gets', function (list) {
            var otbl = $('#Table').DataTable();
            if (list)
                $.each(list, function (i, item) {
                    otbl.rows.add([[item.statusFlag, item.name, 'TRY', item.bank, item.title, item.iban, item.id, item.id]]);
                })
            otbl.draw();
        }).fail(function () {
            onFailure();
