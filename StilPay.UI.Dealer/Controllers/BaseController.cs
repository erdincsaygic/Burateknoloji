...
namespace StilPay.UI.Dealer.Controllers
{
    public abstract class BaseController<T> : Controller where T : BaseEntity, new()
    {
        ...
        protected string IDCompany
        {
            get
            {
                var claim = _httpContext.HttpContext.User.FindFirst(f => f.Type == ClaimTypes.GroupSid);
                var idCompany = claim.Value;
        ...
        protected string Name
        {
            get
            {
                var claim = _httpContext.HttpContext.User.FindFirst(f => f.Type == ClaimTypes.NameIdentifier);
        ...
        protected string Email
        {
            get
            {
                var claim = _httpContext.HttpContext.User.FindFirst(f => f.Type == ClaimTypes.Email);
        ...
        {
            var list = GetData(new FieldParameter("IDCompany", Enums.FieldType.NVarChar, IDCompany));

            return Json(list);
        }

        [HttpPost]
        public virtual IActionResult Gets(JObject jObj)
        {
            var list = GetData();
