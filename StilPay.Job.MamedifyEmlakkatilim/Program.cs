...
namespace StilPay.Job.MamedifyEmlakkatilim
{
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var serviceIDSUG = startUp.EmlakkatilimApi.serviceIDSUG;
            var serviceIDTGC = startUp.EmlakkatilimApi.serviceIDTGC;
            ...
            var companyBankAccountID = startUp.EmlakkatilimApi.companyBankAccountID;
            Console.WriteLine(
                    string.Concat("-------------------------------------------------",
                                   Environment.NewLine, Environment.NewLine,
                                  "            STILPAY MAMEDIFY-EMLAKKATILIM-BANK WORKING..            ",
                                   Environment.NewLine, Environment.NewLine,
                                  "-------------------------------------------------")
            );
            ...
            while (true)
            {
                ...
                try
                {
                             ...
                             .OrderByDescending(o => o.SystemDate))
                    {
                        ...
                        var desc = hareket.Description;

                        if (desc.IndexOf("CSS", StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            dynamicServiceID = serviceIDTGC;
                            phone = "05065325507";
                        }
                        else if (desc.IndexOf("SUG", StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            dynamicServiceID = serviceIDSUG;
                            phone = "05394349570";
                        }
                        else if (desc.IndexOf("SCL", StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                        ...
                        if (!string.IsNullOrEmpty(dynamicServiceID)) 
                        {
                            ...
                            var transactionNr = tSQLBankManager.AddNotificationTransaction(
                                DateTime.Now,
                                tranDate, tranDate,
                                bankId, dynamicServiceID, transactionId,
                            ...
                            if (!string.IsNullOrEmpty(transactionNr))
                            {
                                var pyID = tSQLBankManager.AddAutoPaymentNotification(
                                    tranDate, bankId, senderName, dynamicServiceID, transactionId,
                                ...
                                if (pyID != null && IDOutAuto != null)
                                {
                                    ...
                                    foreach (var item in phones)
                                    {
                                        ...
                                        string msg = $"TR230021100000082947800003 {formattedDate} {senderName} {formattedAmount}";
  ...
  <soapenv:Body>
    <ser:GetAccountStatement>
      <ser:request>
        ...
        private static string ExtractKeyValue(string text, params string[] keys)
        {
            if (string.IsNullOrWhiteSpace(text)) return null;

            var keysPattern = string.Join("|", keys.Select(Regex.Escape));
            var m = Regex.Match(
                text,
                $@"(?i)(?:{keysPattern})\s*[:=]\s*(?<val>.*?)(?=\s+[A-Za-zÇĞİÖŞÜçğıöşü\.]+\s*[:=]|$)",
                RegexOptions.CultureInvariant);

            return m.Success ? m.Groups["val"].Value.Trim() : null;
        }

        private static bool ContainsTokenTr(string text, string token)
        {
            var tr = CultureInfo.GetCultureInfo("tr-TR");
            return tr.CompareInfo.IndexOf(text ?? "", token ?? "", CompareOptions.IgnoreCase) >= 0;
        }
