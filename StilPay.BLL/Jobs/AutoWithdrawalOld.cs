...
namespace StilPay.BLL.Jobs
{
    ...
    public class AutoWithdrawalOld : IInvocable
    {
        ...
        public AutoWithdrawalOld(ICompanyWithdrawalRequestManager companyWithdrawalRequestManager, ICompanyManager companyManager, ISystemSettingManager systemSettingManager, ICompanyIntegrationManager companyIntegrationManager, ICallbackResponseLogManager callbackResponseLogManager, ICompanyBankAccountManager companyBankAccountManager)
        {
        ...
        public async Task Invoke()
        {
            ...
            //    new FieldParameter("Status", Enums.FieldType.Tinyint, 1),
            ...
            //                    MoneyTransferDescription = string.IsNullOrWhiteSpace(item.DealerDescription) ? $"{item.Company} Çekim Talebi" : item.DealerDescription,
            ...
            //                    item.MDate = DateTime.Now;
            //                    item.MUser = "00000000-0000-0000-0000-000000000000";
            //                    item.IsBankQueryNr = kuveytTurkTransferResponse.Data.value.moneyTransferTransactionId.ToString();
            //                    item.SIDBank = "07";
            //                    item.CompanyBankAccountID = companyBankAccountID;
            //                    var response = _companyWithdrawalRequestManager.SetStatus(item);
            //                    if (response.Status == "OK")
            ...
            //                            status_type = 2,
            //                            ciphered = tMD5Manager.EncryptBasic(companyIntegration.SecretKey),
            //                            data = new { transaction_id = item.RequestNr, sp_transactionNr = item.TransactionNr, amount = item.Amount, sp_id = item.ID, message = kuveytTurkTransferResponse.Message }
            //                        };
